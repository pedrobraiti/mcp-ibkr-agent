...
def _warn_if_daily_cap_off(settings: Settings) -> None:
    """Warn loudly when there is no daily spend cap while live trading is armed.

    ``MAX_DAILY_VALUE`` defaults to None (no cap), so out of the box the only dollar
